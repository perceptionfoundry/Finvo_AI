...
async def extract_from_base64(request: Dict[str, Any]):
    """
    Extract financial data from base64 encoded image.
    ...
    if "image_data" not in request:
        raise HTTPException(status_code=400, detail="image_data field required")
    
    try:
        agent = InvoiceExtractionAgent()
        
        filename = request.get("filename", "base64_image.png")
        extract_fuel_info = request.get("extract_fuel_info", True)
        extract_line_items = request.get("extract_line_items", True)
        ...
        # Perform extraction
        result = agent.extract_from_base64(
            request["image_data"],
            filename,
            extraction_request
        )
        ...
        logger.warning(
            "Base64 extraction failed with known error",
        ...
        logger.error(
            "Unexpected error during base64 extraction",
            filename=request.get("filename", "unknown"),
            error=str(e),
            exc_info=True
        )
